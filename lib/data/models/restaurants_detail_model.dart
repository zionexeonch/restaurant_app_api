class RestaurantDetailResult {
  ...
  factory RestaurantDetailResult.fromJson(Map<String, dynamic> json) =>
      RestaurantDetailResult(
        error: json["error"] is bool ? json["error"] : json["error"] == 1,
...
class Menus {
  ...
  List<Foods> foods;
  List<Drinks> drinks; // Change from List<Category> to List<Drinks>

  factory Menus.fromJson(Map<String, dynamic> json) => Menus(
        foods: List<Foods>.from(json["foods"].map((x) => Foods.fromJson(x))),
        drinks: List<Drinks>.from(
          // Change from List<Category> to List<Drinks>
          json["drinks"].map((x) => Drinks.fromJson(x)),
        ),
      );
